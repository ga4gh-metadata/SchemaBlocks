...
END
    my $prop_md;
    my @prop    =   sort keys %{ $attr{$class}->{properties} };
    my @prop_s  =   @{ $attr{$class}->{info}->{sortorder} };
    ...
    foreach my $property (@prop_s) {
      ...
      # making sure no non-existing attributes are being used
      if (! grep{ $_ eq $property } @prop) { next }
